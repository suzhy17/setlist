...
<div class="container" id="body">
	...
	<div id="categoryType" class="btn-group btn-group-justified" role="group" aria-label="이모티콘 종류 선택">
		<div class="btn-group" role="group">
			<a href="/emoticon/mms/products?write&type=template" class="btn ${type == 'template' ? 'btn-info' : 'btn-default'} disabled">템플릿</a>
		</div>
		<div class="btn-group" role="group">
			<a href="/emoticon/mms/products?write&type=icon" class="btn ${type == 'icon' ? 'btn-info' : 'btn-default'} disabled">아이콘</a>
		</div>
		<div class="btn-group" role="group">
			<a href="/emoticon/mms/products?write&type=texticon" class="btn ${type == 'texticon' ? 'btn-info' : 'btn-default'} disabled">그림문자</a>
		</div>
	</div>
	<br>

	<form id="product-form" method="post">
		<input type="hidden" name="productTypeCd" value="${product.productTypeCd}" />
		...
		<table class="table table-bordered">
			<tbody>
				<tr>
					...
					<td>
						<div class="col-md-4">
							<input type="text" name="contentNm" class="form-control" maxlength="20" placeholder="템플릿명" value="${product.contentNm}" />
						</div>
						...
						<div class="col-md-4">
							<select name="categoryCd" class="form-control">
								...
								<c:forEach var="item" items="${usedCategoryList}">
									<option value="${item.categoryCd}" ${item.categoryCd == product.categoryCd ? 'selected="selected"' : ''}>${item.categoryNm}</option>
								</c:forEach>
								<c:forEach var="item" items="${unusedCategoryList}">
									<option value="${item.categoryCd}" ${item.categoryCd == product.categoryCd ? 'selected="selected"' : ''}>${item.categoryNm} (미사용)</option>
								</c:forEach>
							</select>
						</div>
					</td>
				...
				<tr>
					...
					<td>
						<div class="col-md-2 text-center">
							<img src="/common/Image.do?fileName=${bgFile.imgPath}" style="max-height: 100px; max-width: 100px;">
						</div>
						<div class="col-md-2">
				...
				<tr>
					...
					<td>
						<div class="col-md-2 text-center">
							<img src="/common/Image.do?fileName=${thumbFile.imgPath}" style="max-height: 100px; max-width: 100px;">
						</div>
						<div class="col-md-2">
							<input type="file" id="file6" name="file6" class="form-control" />
						</div>
						<div class="col-md-4">
							<span class="slim">(240x360)</span>
						</div>
					</td>
				</tr>
				<tr>
					...
					<td>
						...
						<div id="imageFormArea" class="col-md-9">
							<c:forEach var="item" items="${addFileList}" varStatus="status">
							<div class="row tmpl">
								<div class="col-md-2 text-center files">
									<canvas width="100" height="100" style="background: url('/common/Image.do?fileName=${item.imgPath}')"></canvas>
								</div>
								<div class="col-md-9">
									<div class="row uploaded">
										<span class="type-title" data-src="${item.imgPath}"><strong>${item.sizeCode}</strong></span>
										<ul>
											<li><strong>원본파일</strong> : <span class="org-file">${item.originFileNm}</span><br/>
											<li><strong>저장파일</strong> : <span class="saved-file">${item.imgPath}</span>
										</ul>
									</div>
								</div>
								<hr />
				...
				<tr>
					...
					<td>
						<div class="col-md-12">
							<textarea name="templateSource" rows="15" class="form-control">${product.templateSource}</textarea>
						</div>
					</td>
				</tr>
				<tr>
					...
					<td>
						<div class="col-md-4">
							<input type="text" name="tagNm" class="form-control" maxlength="20" placeholder="태그" value="${product.tagNm}" />
						</div>
				...
				<tr>
					...
					<td>
						<label class="radio-inline">
							<input type="radio" name="saleYn" value="Y" ${product.saleYn == 'Y' ? 'checked="checked"' : ''}> 예
						</label>
						<label class="radio-inline">
							<input type="radio" name="saleYn" value="N" ${product.saleYn == 'N' ? 'checked="checked"' : ''}> 아니오
						</label>
					</td>
				</tr>
			</tbody>
	...
	$(document).ready(function() {
		...
		$frm.ajaxForm({
			...
			beforeSubmit: function(formData, jqForm, options) {
				...
				$.blockUI({
					message: '처리 중 입니다.<br/>잠시만 기다려주세요.'
				});
		...
		var $templateSource = $('textarea[name="templateSource"]'),
			$xml = $($templateSource.text());
...
//				htmlTag += 'isHue="" ';
		...
		var depth = $xml.find('icon').length+10;
